app.factory("$win", ["$compile", "$q", "$rootScope", "$modal", function ($compile, $q, $rootScope, $modal) {
    return {
        prompt: function (options) {

            var DEFAULTS = {
                templateHeader:
                '<form class="form-horizontal" w5c-form-validate=""  name="validateForm">'+
        ...
        confirm: function (options) {
            var DEFAULTS = {
                img: "images/components/alert/alert-question.png",//图标
                content: "",//内容
                title: "",//内容标题
            ...
            var options = $.extend({}, DEFAULTS, options);
            var modalInstance = $modal.open({
                templateUrl: "common/views/confirm.html",
        ...
        alert: function (option) {
            ...
            angularDomEl.attr({
                "template-url": "common/template/alert/alertFixed.html",
                "type": config.type,
                "size": config.size,
                "close": "close(element)"
            });
